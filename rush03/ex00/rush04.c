...
/*   Updated: 2016/08/27 20:59:55 by mhorn            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rush.h"

void	corners(int row, int col, int last_row, int last_col)
{
	if (row == 1 && col == 1)
		put_letter('A');
	else if (last_row == 1 || last_col == 1)
		put_letter('C');
...
void	print_row(int last_col, int last_row, int row)
{
	...
	while (col <= last_col)
	{
		...
		{
			corners(row, col, last_row, last_col);
		}
